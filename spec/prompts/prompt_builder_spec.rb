...
RSpec.describe PromptBuilder, type: :model do
  describe '#worker_prompt' do
    ...
    let(:prompt_builder) { PromptBuilder.new(custom_prompts: custom_prompts, goal: goal, summary: summary, last_actions: last_actions) }
    ...
    it 'includes goal' do
      expect(prompt_builder.worker_prompt).to include(goal)
    end
